...
class SortingRobot:
    ...
    def sort(self):
        ...
        if self.compare_item() == None:
          print('swaped item', self._item, self._list[self._position])
          self.swap_item()
          self.move_right()
        elif self.compare_item() == -1:
          print('item held less than item in front', self._item, self._list[self._position])
          while self.can_move_left() and self.compare_item() != None:
            self.move_left()
            print('moved left', self._position)
            # self.swap_item()
            # self.move_right()
        elif self.compare_item() == 1:
          self.swap_item()
          print('item is less than item being held', self._item, self._list[self._position])
